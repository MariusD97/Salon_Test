...
function renderTodayAgenda() {
  ...
  if (isEditingHere) {
    ...
    els.todayAgenda.innerHTML = `<div class="agenda-form card"><div class="day-card-header"><h2 class="day-card-title">${appointment ? "Editează programarea" : "Programare nouă"}</h2><span class="agenda-form-date">${agendaDateLabel(agendaDate)}</span></div>${formHtml(appointment)}</div>`;
...
function formHtml(appt) {
  ...
  return `<form id="apptForm"><div class="date-field-wrap"><label class="field">Data programării<input type="date" id="f_date" value="${dateValue}" required></label></div><div class="form-row"><label class="field">Ora<input type="time" id="f_time" value="${appt ? appt.time : "09:00"}" required></label><label class="field">Durata (minute)<input type="number" id="f_duration" min="15" step="15" value="${appt ? appt.duration : 60}" required></label></div><label class="field" style="position: relative;">Numele clientei<input type="text" id="f_client" value="${escapeHtml(clientValue)}" placeholder="Scrie nume sau alege din listă..." autocomplete="off" required><ul id="clientSuggestions" class="suggestions-list" style="display: none;"></ul></label><label class="field">Serviciu<select id="f_service">${SERVICES.map(s => `<option value="${escapeHtml(s.name)}" ${s.name === service ? "selected" : ""}>${escapeHtml(s.name)}${s.price !== "" ? ` — ${s.price} lei` : ""}</option>`).join("")}</select></label><label class="field">Cost (lei)<input type="number" id="f_cost" min="0" step="1" value="${cost}" placeholder="ex: 120"></label><label class="field">Notițe (opțional)<textarea id="f_notes" placeholder="ex: gel roz, model floral">${appt ? escapeHtml(appt.notes || "") : ""}</textarea></label><div id="f_error"></div><div class="form-actions"><button type="button" class="cancel-btn" id="f_cancel">Renunță</button><button type="submit" class="save-btn">${appt ? "Salvează modificarea" : "Salvează programarea"}</button></div></form>`;
}
